...
int main(int argc, char *argv[]) {
    ...
    if (list_sectors) {
        ...
        if (fsize > (long)s->size) {
            fprintf(out, "Error: File too large for sector (%ld > %llu)\n", fsize, s->size);
            free(content);
            goto cleanup;
        }
        ...
        kern_return_t kr = task_for_pid(mach_task_self(), target_pid, &task);
        if (kr != KERN_SUCCESS) {
            fprintf(out, "Error: task_for_pid failed (Error: %d)\n", kr);
            free(content);
            goto cleanup;
        }

        vm_prot_t new_prot = VM_PROT_READ | VM_PROT_WRITE;
        if (!(s->protection & VM_PROT_WRITE)) {
            kr = mach_vm_protect(task, (mach_vm_address_t)s->base, s->size, FALSE, new_prot);
            if (kr != KERN_SUCCESS) {
                fprintf(out, "Error: mach_vm_protect failed to make writable (Error: %d)\n", kr);
                free(content);
                mach_port_deallocate(mach_task_self(), task);
                goto cleanup;
            }
